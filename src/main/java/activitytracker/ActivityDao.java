...
public class ActivityDao {
    ...
    private long getIdByStatement(PreparedStatement statement) {
        try (ResultSet resultSet = statement.getGeneratedKeys()) {
            ...
            throw new IllegalStateException("Nem kaptam meg az ID-t!");
        } catch (SQLException se) {
            throw new IllegalStateException("Már megint elbaszódott valami!", se);
        }
    }
    ...
    private Activity getActivityByResultSet(PreparedStatement statement) throws SQLException {
        try (ResultSet resultSet = statement.executeQuery()) {
            if (resultSet.next()) {
                ...
                throw new IllegalArgumentException("Nem találom ezt az ID-t!");
            }
        }
    }
